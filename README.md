# Projeto Final IHC - Dupla João Victor de Sousa Leite e Artur De Araújo Negreiros 

# Descrição do Projeto

Muitas pessoas perdem o controle financeiro e gastam mais do que deveriam. Para elas poderem ter mais controle de seus gastos e terem mais conhecimento de suas finanças em geral para economizar melhor, essas pessoas podem usar um app que registra receitas e despesas, vê gráficos do quanto gastou por categoria (alimentação, transporte etc.), e recebe alertas quando estiver se aproximando de um limite mensal.

## Funcionalidades principais:
- **Funcionalidade 1**: Cadastro de usuários
- **Funcionalidade 2**: Monitoramento de finanças
- **Funcionalidade 3**: Visualização de despesas

---

# Personas do Projeto

A criação de personas foi essencial para entendermos as necessidades e comportamentos dos usuários que irão utilizar o sistema. Abaixo está o link das personas do projeto:

[Personas](https://www.canva.com/design/DAG2bZa64Vw/A413C7t4LxLkdm5ytJDNRg/edit)
![Personas](https://github.com/jvleiteifpi/trabalho-final-ihc/blob/main/assets/image/perso-01.png?raw=true)
![Personas](https://github.com/jvleiteifpi/trabalho-final-ihc/blob/main/assets/image/perso-02.png?raw=true)


---

# Protótipo de Baixa Fidelidade

O protótipo de baixa fidelidade tem como objetivo representar as ideias principais do design de forma simples e rápida. Ele ajuda a visualizar o fluxo de navegação e os principais elementos da interface, sem se preocupar com detalhes de design.

![Protótipo Baixa Fidelidade](https://github.com/jvleiteifpi/trabalho-final-ihc/blob/main/assets/image/baixa_fidelidade.jpg?raw=true)

---

# Protótipo de Alta Fidelidade

A versão de alta fidelidade do protótipo traz um design mais elaborado, com cores, tipografia e outros elementos visuais mais refinados. Esse protótipo proporciona uma visão mais próxima do produto final.

[Protótipo Alta Fidelidade](https://www.figma.com/design/pbnBMFSMhFD6XYJSU96RzP/Untitled?node-id=0-1&t=7V5EqpsBoy2VIgdJ-1)

![Protótipo Alta Fidelidade1](https://github.com/jvleiteifpi/trabalho-final-ihc/blob/main/assets/image/alta-fide-01.png?raw=true)
![Protótipo Alta Fidelidade2](https://github.com/jvleiteifpi/trabalho-final-ihc/blob/main/assets/image/alta-fide-02.png?raw=true)
![Protótipo Alta Fidelidade3](https://github.com/jvleiteifpi/trabalho-final-ihc/blob/main/assets/image/alta-fide-03.png?raw=true)

---

# Forms
[Forms](https://forms.gle/kJZZq9QtpGSVD7ZN9)

---

# Implementação
Segue no próprio Github o código referente a implementação da aplicação em voga.

